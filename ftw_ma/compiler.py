...
def fit(config, ckpt_path, cli_args):
    ...
    default_root_dir = (
        fit_config.get("trainer", {}).get("default_root_dir")
    ...
    if default_root_dir:
        ...
        os.makedirs(os.path.join(default_root_dir, "best_samples"), exist_ok=True)
        print(f"Ensured run directories exist under: {default_root_dir}")
...
def test(
    ...
    batch_size = data_args.get("batch_size", 64)
    ...
    print(
        f"Created dataloader with {len(ds)} samples in "
